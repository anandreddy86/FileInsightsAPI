...
public class FileService {
    ...
    private void processFile(File file) {
        try {
            // Step 1: Extract basic file metadata
            FileMetadata fileMetadata = fileMetadataService.extractMetadata(file);
            fileMetadataService.saveFileMetadata(fileMetadata);

            // Step 2: Extract advanced metadata using Tika
            TikaMetadata tikaMetadata = TikaUtils.extractTikaMetadata(file);
